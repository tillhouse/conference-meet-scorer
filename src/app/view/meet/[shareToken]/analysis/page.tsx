...
export default async function PublicMeetAnalysisPage({
  params,
  searchParams,
}: {
  ...
  const { meetLineups, meetTeams, relayEntries } = getComputedMeetView(
    meetForCompute,
    view,
    realResultsEventIds
  );
  ...
  const hasResults =
    meetLineups.some((l) => l.place !== null) ||
    relayEntries.some((r) => r.place !== null);

  const athleteIdToName = new Map<string, string>();
  const pointsByAthleteId = new Map<string, number>();
  meetLineups.forEach((l) => {
    const a = (l as { athlete?: { id?: string; firstName?: string | null; lastName?: string | null } }).athlete;
  ...
  const teamsWithTestSpot = meetTeams.filter((mt) => {
  ...
  const teamsWithSensitivity = meetTeams.filter((mt) => {
    ...
    <div className="space-y-6">
      ...
      {!hasResults ? (
        <Card>
          <CardContent className="pt-6">
            <p className="text-slate-600">
              Run Simulate to see analysis results. After simulating, test spot and sensitivity
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-slate-600">
              No analysis configured. Set a test spot or sensitivity athlete on Set Rosters, then
              run Simulate.
            </p>
          </CardContent>
        </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {teamsWithTestSpot.map((mt) => {
                  ...
                  const scoringId =
                    (mt as { testSpotScoringAthleteId?: string | null }).testSpotScoringAthleteId ??
                    testSpotIds[0];
                  ...
                  const sorted = [...testSpotIds].sort(
                    (a, b) =>
                      (pointsByAthleteId.get(b) ?? 0) - (pointsByAthleteId.get(a) ?? 0)
                  );
                  return (
                    <div
                      key={mt.id}
                      ...
                      style={
                        mt.team.primaryColor
                          ? { borderLeftWidth: 4, borderLeftColor: mt.team.primaryColor }
                          : undefined
                      }
                    >
                        ...
                        {sorted.map((aid) => {
                          ...
                          return (
                            <span
                              key={aid}
                              className={
                                isScoring ? "font-medium text-slate-900" : "text-slate-600"
                              }
                            >
                              {name} {pts.toFixed(1)}
                      ...
                      {sorted.map((aid) => {
                        const lineups = meetLineups.filter(
                        ...
                        return (
                          <div key={aid} className="mt-4">
                            <div
                              className={`text-sm font-medium ${isScoring ? "text-slate-900" : "text-slate-700"}`}
                            >
                              {name}
                            </div>
                              ...
                              <table className="w-full text-sm border-collapse analysis-table">
                                ...
                                <thead>
                                  <tr className="border-b bg-slate-50 font-medium text-slate-700">
                                    ...
                                    <th className="text-right py-1.5 font-medium w-[6rem]">
                                      Points
                                    </th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {lineups.map((l) => {
                                    const timeStr =
                                      (l as { overrideTime?: string | null }).overrideTime ??
                                      l.seedTime;
                                  ...
                                  <tr className="border-t font-medium text-slate-900">
                                    ...
                                    <td className="text-right py-2 w-[6rem]">
                                      {subtotal.toFixed(1)}
                                    </td>
                                  </tr>
                                  <tr className="border-t-2 border-slate-200 font-medium text-slate-900">
                                    ...
                                    <td className="text-right py-2 pr-3 w-[7rem]" />
                                    <td className="text-right py-2 w-[6rem]">
                                      {teamTotalWhenScoring.toFixed(1)}
                                    </td>
                                  </tr>
                                </tbody>
                              </table>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {teamsWithSensitivity.map((mt) => {
                  const sensPct =
                    (mt as { sensitivityPercent?: number | null }).sensitivityPercent ?? 1;
                  ...
                  const results: SensResult[] = resultsRaw
                    ? (JSON.parse(resultsRaw) as SensResult[])
                    : [];
                  return (
                    <div
                      key={mt.id}
