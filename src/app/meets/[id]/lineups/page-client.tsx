...
interface MeetLineupsPageClientProps {
  meetId: string;
  meetTeams: MeetTeam[];
  swimmingEvents: any[];
  divingEvents: any[];
...
export function MeetLineupsPageClient({
  meetId,
  meetTeams,
  swimmingEvents,
  divingEvents,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {filteredTeams.map((meetTeam) => (
          <LineupSelector
            ...
            maxIndivEvents={maxIndivEvents}
            maxRelays={maxRelays}
            maxDivingEvents={maxDivingEvents}
          />
        ))}
      </div>
      ...
      <LineupNavigation
        ...
        backUrl={`/meets/${meetId}/roster`}
        nextUrl={`/meets/${meetId}/relays`}
      />
    </div>
  );
}
